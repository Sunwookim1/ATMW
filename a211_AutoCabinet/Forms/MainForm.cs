using ATMW.Class;
using ATMW.Class.Sound;
using ATMW.Controls;
using ATMW.Datas;
using ATMW.Properties;
...
{
    public partial class ATMW_Main : Form, ReaderEventListener
    {

        private static readonly string TAG = typeof(ATMW_Main).Name;
        ...
        public ATMW_Main(/*int userDataID*/)
        {
            ...
            // 태그 리스트 가져오는 메서드 등록
            ApiDefinit.GetTagListEvent += new ApiDefinit.GetTagList(GetTagDataList);
        }
                                ...
                                string port)
        {
            ...
            if (ApiDefinit.NetWorkModeCheck)
            {
            ...
            UpdateTagList(port, epc, rssi); // GateMode
            ...
            if (GateMode_Port_Change)
            {
                port = TagInfoList[InfoCount].Port;
                g_TagCurAntNo = Convert.ToInt32(TagInfoList[InfoCount].Port) - 1;
            }


            UpdateList(ApiDefinit.DeviceName, port, epc, rssi);

            if (!GateModeCheck)
            {
                bool Compareresult = TagCompareRssi(epc, rssi, CurentPort);
                if (!Compareresult)
                    return;
            }

            bool ReadingCountCheck = ReadingCount(epc);

            if (ReadingCountCheck && GateModeCheck)
            {
        ...
        {
            ApiDefinit.DeviceName = DeviceName;
            ApiDefinit.GsUri = GsUri;
            ApiDefinit.ApiUri = ApiUri;
            ApiDefinit.DeviceId = DeviceId;
            ApiDefinit.NetWorkModeCheck = NetWorkCheck;
            ...
            if (SharedValues.Reader == null)
            {
                ...
                if (SharedValues.Reader != null)//Asyen : a213과 연결이 되지 않은 상태일 때
                {
                    ...
                    if (await SharedValues.Reader.StartAsync().ConfigureAwait(true))//Asyen : a213과 연결에 성공했을 경우 
                    {
                        ...
                        HartBitTimer.Interval = 180000;    //3분
                        HartBitTimer.Start();
        ...
        private async void buttonRfidInventory_Click(object sender, EventArgs e)
        {
            ...
            if (ApiDefinit.NetWorkModeCheck)
                ...
                ApiDefinit.DeviceIdRequest();

            await ToggleRfidInventory().ConfigureAwait(true);

            if (ApiRequestTimeInterval != 0)
            {
        ...
        // API 호출 데이터 저장
        List<SaveAPI> SaveApiData = new List<SaveAPI>();

        private void UpdateBufferLocation()
        {
            // 한 사이클이 끝나고 데이터를 서버에 올려줄때의 시간
            string CycleEndTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            ...
            for (int i = 0; i < AntCount; i++)
            {
                ...
                else if (g_TagBufferData[i, BeforeBufferLocation, 0] != null && g_TagBufferData[i, g_TagBufferCurLocationArray[i], 0] == null)
                {
                    for (int j = 0; j < g_TagBufferData[i, BeforeBufferLocation, 0].Length; j++)
                    {
                        if (ApiDefinit.NetWorkModeCheck)
                        {
                            if (ApiRequestTimeInterval == 0 && ContinuetoApplyCheck)
                            {
                                ApiDefinit.RequestJSON2(ApiDefinit.DeviceId, "anonymous", i + 1, CycleEndTime, g_TagBufferData[i, BeforeBufferLocation, 0][j], --Count[i], 0, ++OutputCount[i]);
                            }
                            else
                                SaveApiData.Add(new SaveAPI("RequestJSON2", ApiDefinit.DeviceId, "anonymous", i + 1, CycleEndTime, g_TagBufferData[i, BeforeBufferLocation, 0][j], --Count[i], 0, ++OutputCount[i]));
                        }
                        RemoveTagInfo(g_TagBufferData[i, BeforeBufferLocation, 0][j]);
                    }
                ...
                if (g_TagBufferData[i, BeforeBufferLocation, 0] == null && g_TagBufferData[i, g_TagBufferCurLocationArray[i], 0] != null)
                {
                    ...
                    for (int j = 0; j < g_TagBufferData[i, g_TagBufferCurLocationArray[i], 0].Length; j++)
                    {
                        if (ApiDefinit.NetWorkModeCheck)
                        {
                            if (ApiRequestTimeInterval == 0 && ContinuetoApplyCheck)
                            {
                                ApiDefinit.RequestJSON1(ApiDefinit.DeviceId, "anonymous", i + 1, CycleEndTime, g_TagBufferData[i, g_TagBufferCurLocationArray[i], 0][j], ++Count[i], 0, ++InputCount[i]);
                            }
                            else
                                SaveApiData.Add(new SaveAPI("RequestJSON1", ApiDefinit.DeviceId, "anonymous", i + 1, CycleEndTime, g_TagBufferData[i, g_TagBufferCurLocationArray[i], 0][j], ++Count[i], 0, ++InputCount[i]));
                        }
                    }
                    tagDataViews[i].labelStateIN.BackColor = Color.Red;
                }
        ...
        private void devSettingToolStripMenuItem_Click(object sender, EventArgs e)
        {
            using (SettingDefaultForm dlg = new SettingDefaultForm(this, CultureString, ApiDefinit.DeviceId, ApiDefinit.NetWorkModeCheck))
            {
        ...
        private void modeSettingToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            using (DeviceNotificationListForm form = new DeviceNotificationListForm(ApiDefinit.DeviceId))
            {
        ...
        // 업데이터 시간 설정 폼 로드                                                                                                                         
        private void updateTimeSettingToolStripMenuItem_Click(object sender, EventArgs e)
        {
            using (UserTimeSettingForm form = new UserTimeSettingForm())
            {
                form.UpdateDateTime += new UserTimeSettingForm.SettingUpdateDateTime(LoadUpDateTime);
                form.UpdateContinueDateTime += new UserTimeSettingForm.SettingContinueDateTime(SetTimeInterval);
                if (form.ShowDialog() == DialogResult.OK)
                {
        ...
        private bool ContinuetoApplyCheck = true;
        private int ApiRequestTimeInterval = 0;

        private void LoadUpDateTime(DateTime DateTime, bool ApplyCheck)
        {
            this.UpdateDateTime = DateTime;
            this.ContinuetoApplyCheck = ApplyCheck;

            ConvertDatetoMiliSecond(UpdateDateTime);

        }

        private void ConvertDatetoMiliSecond(DateTime UpdateDateTime)
        {
            long currenttime = (long)DateTime.Now.ToUniversalTime().Subtract(new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc)).TotalMilliseconds;
            long UpdateTime = (long)UpdateDateTime.ToUniversalTime().Subtract(new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc)).TotalMilliseconds;

            long ResultTime = UpdateTime - currenttime;

            ApiRequestTimeInterval = Convert.ToInt32(ResultTime);
            ...
            // 리더가 동작 중이라면
            // 실시간으로 타이머 활성화 및 인터벌 적용
            if (mRfidInventoryStarted)
            {
                APITimer.Interval = ApiRequestTimeInterval;
                APITimer.Enabled = true;
            }
            else
            {
        ...
        private void SetTimeInterval(int TimeInterval, bool ApplyCheck)
        {
            ApiRequestTimeInterval = TimeInterval;
            ContinuetoApplyCheck = ApplyCheck;
        ...
        private void HartBitTimer_Tick(object sender, EventArgs e)
        {
            if (ApiDefinit.NetWorkModeCheck)
                ApiDefinit.RequestHartBit(ApiDefinit.DeviceId, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
        }

        private void gateModeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            GateModeCheck = true;
        }
    }
